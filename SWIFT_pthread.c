...
void contract(T_t * X, T_t * Y, outputT_t * Z, pointer_type CMODES, pointer_type * CMODES_X, pointer_type * CMODES_Y) {
	printf("\nPrinting Tnsr X:\n");
	print_tensor_type(X);
	printf("\nPrinting Tnsr Y:\n");
	print_tensor_type(Y);
	...
	printf("FMODES_X: "); print_array(FMODES_X,XFMODES);
	printf("FMODES_Y: "); print_array(FMODES_Y,YFMODES);
	...
	printf("Xshape: "); print_array(X->shape, X_order);
	printf("Yshape: "); print_array(Y->shape, Y_order);
	printf("contracting_shape: "); print_array(contracting_shape,CMODES);
	...
	//printf("output_shape: "); print_array(Z_shape, Z_order);
	...
	//DEFINITION OF THREAD FUNCTION TO START CONTRACTION
	typedef struct threadarg { pointer_type index; } threadarg; 
	threadarg args[NUM_THREADS];

	void *thread_function(void *arg){
		threadarg *args = (threadarg *)arg;
		pointer_type idx = args->index;

		if(idx == 0){
		clock_gettime(CLOCK_MONOTONIC, &phase);
		elapsed_us = (phase.tv_sec - start.tv_sec) * 1000000LL + (phase.tv_nsec - start.tv_nsec) / 1000LL;
		preliminary_us = elapsed_us;
		}
		////////////////////////////////////////////////////////////////////////////////////////
		//
		//							PHASE II: PERMUTATION 
		//
		////////////////////////////////////////////////////////////////////////////////////////
		...
		if(idx == 1){
			//CREATING DIVISIONS FOR Y ENTRIES
		...
		pthread_mutex_lock(&barrier); SNYdone++; Yfiltered_count += Yfiltered_tmp; pthread_mutex_unlock(&barrier);
		while(SNYdone < 8) {}
		...
		clock_gettime(CLOCK_MONOTONIC, &phase);
		elapsed_us = (phase.tv_sec - start.tv_sec) * 1000000LL + (phase.tv_nsec - start.tv_nsec) / 1000LL;
		permutation_us = elapsed_us - preliminary_us;
		}
		////////////////////////////////////////////////////////////////////////////////////////
		//
		//							PHASE III: INPUT SORTING 
		//
		////////////////////////////////////////////////////////////////////////////////////////
		if(idx == 0){ // SORTING X ENTRIES
		...
		pthread_mutex_lock(&barrier); Xsorting_done++; pthread_mutex_unlock(&barrier);
		while(Xsorting_done < 8){}
		...
		if(idx == 0){
			...
			//DISPLAYING BOUNDS 
			printf("range_divisions: "); print_array(range_divisions, NUM_THREADS+1);
			printf("entry_divisions: "); print_array(entry_divisions, NUM_THREADS+1);
		...
		} // FIRST THREAD RESPONSIBILITY

		if(idx == 0){ //ALSO FIRST THREAD RESPONSIBILITY 
			printf("\n");
			printf("number of r_anges: %d\n",ranges_write_ptr);
			printf("X_entry_count: %d\n",X_entry_count);
			printf("number of duplicate entries: %d\n", Xacc_write_ptr);
			printf("number of non-duplicate entries: %d\n", Xnacc_count);
			printf("number of non-duplicate entries: %d\n", Xnacc_write_ptr);
			printf("total number of entries: %d\n", Xacc_write_ptr+Xnacc_write_ptr);
		...
		clock_gettime(CLOCK_MONOTONIC, &phase);
		elapsed_us = (phase.tv_sec - start.tv_sec) * 1000000LL + (phase.tv_nsec - start.tv_nsec) / 1000LL;
		sorting_us = elapsed_us - permutation_us - preliminary_us;
		};
		...
		pthread_mutex_lock(&barrier); readyToContract++; pthread_mutex_unlock(&barrier);
		while(readyToContract < 8);
		...
		clock_gettime(CLOCK_MONOTONIC, &phase);
		elapsed_us = (phase.tv_sec - start.tv_sec) * 1000000LL + (phase.tv_nsec - start.tv_nsec) / 1000LL;
		contraction_us = elapsed_us - sorting_us - permutation_us - preliminary_us;
		}
		////////////////////////////////////////////////////////////////////////////////////////
		//
		//							PHASE IV: WRITEBACK
		...
		clock_gettime(CLOCK_MONOTONIC, &phase);
		elapsed_us = (phase.tv_sec - start.tv_sec) * 1000000LL + (phase.tv_nsec - start.tv_nsec) / 1000LL;
		writeback_us = elapsed_us - accumulation_us - contraction_us - sorting_us - permutation_us - preliminary_us;
		}
	...
	for(pointer_type i = 0; i < NUM_THREADS; ++i){
		args[i].index = i;
	...
	for(pointer_type i = 0; i < NUM_THREADS; ++i){
		if (pthread_join(threads[i], NULL) != 0) {
			perror("pthread_join() error"); exit(3);
		}
	}
	...
	printf("\n");
	printf(" Preliminary: %d us\n", preliminary_time );
	printf(" Permutation: %d us\n", permutation_time );
	printf("     Sorting: %d us\n", sorting_time     );
	printf(" Contraction: %d us\n", contraction_time );
	printf("   Writeback: %d us\n", writeback_time   );
...
int main(int argc, char ** argv) {
	...
	pointer_type CMODES_X[] = {1,2};
	pointer_type CMODES_Y[] = {1,2};

	contract(X, Y, Z, CMODES, CMODES_X, CMODES_Y);
